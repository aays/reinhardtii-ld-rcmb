...
fnames <- dir_ls('data/ldhelmet/block_5/', regexp = 'chromosome_[0-9]+_summarised\\.txt')
...
hotspot_groups %>% 
  ...
  summarise(mean_length = mean(length)) # 3298 bp
```
...
hotspot_sequence / genome_length # 0.0307
```
...
hotspot_distances %>% 
  summarise(mean_distance = mean(distance, na.rm = TRUE)) # 104.73 kb
```
