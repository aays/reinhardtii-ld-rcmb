...
correlates %>% 
  map_dfr(~ calculate_enrichment(annot_genome, .) %>% 
        broom::tidy(), 
        .id = 'correlate'
  )
...
annot_genome %>% 
  ...
  filter(correlate %in% c('upstream', 'downstream', 'both')) %>% 
  summarise_at(vars(contains('prop')), sum)
```

17.5% of hotspots are in the 8.8% of sequence corresponding to these annotations.
